"""
Final evaluation: Quantum v6 vs Classical v3
Both trained on mgmno_1000 (112k samples) — fair apples-to-apples comparison.
...
# Both models trained on same dataset for fair comparison
DATASET    = os.path.join(Q_DIR, 'datasets/mgmno_1000.pickle')
Q_CKPT     = os.path.join(Q_DIR, 'results_crystal_qgan_v6/checkpoint_490.pt')
CLS_GEN    = os.path.join(CLS_DIR, 'model_cwgan_mgmno_v3/generator_490')
OUT        = os.path.join(Q_DIR, 'results_eval_v6v3/eval_report.txt')
...
print(f'Quantum v6 loaded: {Q_CKPT}', flush=True)

# ── Classical v3 generator ────────────────────────────────────────────────────
...
q_ehull = run(q_coords, labels_gen, q_valid, 'QUANTUM-v6')
c_ehull = run(c_coords, labels_gen, c_valid, 'CLASSICAL-v3')

# ── Report ────────────────────────────────────────────────────────────────────
def stats(ehull, idx_arr, n):
    cap = min(len(idx_arr), MAX_RELAX)
...
Q = stats(q_ehull, q_valid, N_GEN)
C = stats(c_ehull, c_valid, N_GEN)

def pct(x, n): return f'{x}  ({x/n*100:.1f}%)'
sep = '=' * 72

lines = [sep,
  f'  FINAL EVAL  Quantum-v6 vs Classical-v3  [same data: mgmno_1000  N={N_GEN}]',
  sep,
  f'  {"Category":<42}  {"Classical-v3":>12}  {"Quantum-v6":>12}',
  '-'*72,
  f'  {"Valid geometry MIC (>=1.0A)":<42}  {pct(C["valid"],N_GEN):>12}  {pct(Q["valid"],N_GEN):>12}',
  f'  {"Relaxed":<42}  {pct(C["relaxed"],N_GEN):>12}  {pct(Q["relaxed"],N_GEN):>12}',
  '',
  f'  {"-- POST-RELAXATION E_above_hull --":<42}',
  f'  {"  Stable       (< 0.1 eV/at)":<42}  {pct(C["stable"],N_GEN):>12}  {pct(Q["stable"],N_GEN):>12}',
  f'  {"  Near-stable  (0.1-0.5 eV/at)":<42}  {pct(C["near"],N_GEN):>12}  {pct(Q["near"],N_GEN):>12}',
  f'  {"  Metastable   (0.5-2.0 eV/at)":<42}  {pct(C["meta"],N_GEN):>12}  {pct(Q["meta"],N_GEN):>12}',
  f'  {"  High energy  (>= 2.0 eV/at)":<42}  {pct(C["high"],N_GEN):>12}  {pct(Q["high"],N_GEN):>12}',
  '',
  f'  {"E_hull mean [eV/atom]":<42}  {C["mean"]:>12.4f}  {Q["mean"]:>12.4f}',
  f'  {"E_hull min  [eV/atom]":<42}  {C["mn"]:>12.4f}  {Q["mn"]:>12.4f}',
  sep]
